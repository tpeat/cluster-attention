...
class SequenceGenerator:
    ...
    def generate(self, src, src_mask, strategy=DecodingStrategy.GREEDY, k=None, p=None, start=None):
        """
        ...
        with torch.no_grad():
            ...
            out_tokens = torch.full((batch_size, self.max_len), self.pad_token, dtype=torch.long).to(src.device)
            out_tokens[:, 0] = start
